...
**Copy the key to a different account, with sudo.** Fill in "Copy to account" and Portside still logs in as each host's own user, then runs the same script under `sudo -H -u <account>`. That's the only way to reach an account you can't log in as — a key-only service account being bootstrapped is exactly that case — and it's why Ansible's `authorized_key` module pairs its `user:` parameter with `become`. Running *as* the target rather than as root means files are created with the right ownership, so there's no `chown` and nothing for sshd's `StrictModes` to reject. The confirmation warns that sudo is required before anything is contacted, the sudo password is sent once and never retried, and a host that doesn't permit it is reported in sudo's own words rather than as a bare exit code.

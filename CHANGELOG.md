...
**The sheet says which account the key lands in.** A line under the key names it, every host in the list carries a `→ account`, and the field below is prefilled with the detected account when the whole selection agrees on one — so it's visible, and changing it is one word. It stays empty when the selection spans several accounts or includes an aliased host, since prefilling there would quietly retarget hosts that were fine.

**Copy the key to a different account.** A "Copy to account" field installs the key for an account other than the one the host entry logs in as — `svc_ansible` rather than you. It works by logging in as that account, so there's nothing to escalate and nothing to `chown`: `$HOME` is already the right one. It won't spend the host's own password on it, since that password belongs to a different account and offering it would be a failed authentication on every host in the run; it uses a credential profile whose user matches, or runs key/agent-only and fails cleanly. The field says which. An account you can't log in as at all still needs sudo, which isn't built.

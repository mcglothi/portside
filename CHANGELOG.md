...
**Retiring a key is transactional.** `authorized_keys` is copied aside first, and if that copy fails nothing is rewritten at all. The rewrite itself is guarded against interruption — including you pressing Stop — because writing to a file empties it before the new contents arrive, and being interrupted in that gap would leave the host with a truncated `authorized_keys`. The original is restored instead; if even that fails you are told plainly to recover from the backup by hand rather than being shown a success. Stop takes effect between hosts, never in the middle of one.

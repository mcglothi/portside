...
- Fixed: **a remote filename containing a line break could forge an SFTP command.** Paths were quoted before going into an `sftp` batch, which protects the argument — but a batch is newline-delimited, so a name with a carriage return or line feed in it split one intended command into two, and the second half was whatever the filename said. `rm` is among the commands that could be forged that way. Such names can't survive a directory listing either, so they're now rejected at the boundary with a clear message rather than quietly doing something else. Spaces, quotes, backslashes and non-ASCII are unaffected.
- Fixed: **importing a file that listed the same host twice added it twice.** Duplicates were checked against the library but not against the rest of the incoming batch, so a session (or a macro) repeated inside one import file got through as many times as it appeared.
- Fixed: **an imported session could point at a credential profile that doesn't exist here.** Profiles don't travel in an export, so the profile reference arrived pointing at whatever existed on the machine it came from — silently resolving to no credential at all. A reference that doesn't resolve locally is now cleared and saved-password use switched off, so an import can't carry a dangling pointer or quietly fall through to this machine's default profile. Restoring your own backup keeps the reference *and* keeps working, since there the profile really is present and holds the secret.
- **Recording a command no longer rewrites the whole history file.** Every recorded command re-encoded the entire history — connection stats, the log, and up to 5,000 command lines — and replaced the file on disk. With shell integration on, one MultiExec broadcast across a grid meant one full rewrite per pane. Writes are now coalesced into a fixed window and the file is no longer stored pretty-printed; quitting flushes whatever is outstanding. The window is bounded rather than sliding, so a long stream of commands can't postpone the write indefinitely — which is when there'd be most to lose. Clearing history, or opting out of the log or command capture, still writes immediately, dropping any pending write rather than letting it land afterwards and resurrect what was just deleted.
- CI now ratchets the Swift 6 strict-concurrency warning count per file (`Scripts/strict-concurrency-check.sh`). The package is still on `swift-tools-version: 5.9`, so these aren't errors yet — there are 248 of them across 15 files, and that migration is real work. The ratchet doesn't demand it be done; it fails the build if the count *moves* while it's pending — per file, so progress in one can't pay for regressions in another, and in both directions, so a fix and its tightened baseline land in the same commit rather than leaving slack for a later regression to hide in. `README.md` no longer implies the package already builds in Swift 6 language mode.
